...
class SMILESmodel(object):
    ...
    def load_data(self, preprocess=False, stereochem=1., augment=1):
        all_mols = read_smiles_file(self.dataset)  # returns a list of smiles from the csv file
        if preprocess:
            all_mols = preprocess_smiles(all_mols, stereochem)  # canonicalise smiles and keeps stereochem
        self.molecules = all_mols
        # self.smiles = all_mols
        print("%i molecules loaded from %s..." % (len(self.molecules), self.dataset))
        self.maxlen = max([len(m) for m in self.molecules]) + 2  # longest SMILES...
        print("Maximal sequence length: %i" % (self.maxlen - 2))
        print("Creating InChI keys...")
        self.inchi = inchikey_from_smileslist(
            all_mols)  # converts to RDMol and back to InCHi for novelty comparison later.
        del all_mols
        if augment > 1:
            print("Augmenting SMILES %i-fold..." % augment)
            augmented_mols = randomize_smileslist(self.molecules,
                                                  num=augment)  # generates X different SMILES for the same molecule.
        ...
        self.padded = pad_seqs(["^%s$" % m for m in self.molecules], ' ',
                               given_len=self.maxlen)  # adds SPACES to pad the smiles list
        self.n_mols = len(self.molecules)  # how many smiles we have
        # validation set and training set. Because using sklearn is for noobs.
        # basically it's a fancy way of splitting a % of the dataset
        # self.val_mols, self.train_mols = np.split(np.random.choice(range(self.n_mols), self.n_mols, replace=False), [int(self.validation * self.n_mols)])
        random_indices = np.random.permutation(self.n_mols)
        n_validation_samples = int(self.validation * self.n_mols)
        self.train_mols = random_indices[n_validation_samples:]
        self.val_mols = random_indices[:n_validation_samples]
        print("Using %i examples for training and %i for valdiation" % (len(self.train_mols), len(self.val_mols)))
        self.build_tokenizer() # creates a default token vocabulary
    ...
    def train_model(self, n_sample=100):
        ...
        writer = tf.summary.create_file_writer(log_dir)  # old, but working...
        ...
        while i < self.num_epochs:
            ...
            # here we generate the new smiles and validate them
                ...
                if self.reinforce:  # reinforce = add most similar generated compounds to training pool
                    if len(novel) > (n_sample / 5):
                        if self.mw_filter:
                            ...
                            mw = np.array(
                                [Descriptors.MolWt(MolFromSmiles(s)) if MolFromSmiles(s) else 0 for s in novel])
                        ...
                        if self.reference:  # if a reference mol(s) is given, calculate distance to that one
                            ...
                            fp_train = cats_descriptor([MolFromSmiles(s) for s in self.molecules])
